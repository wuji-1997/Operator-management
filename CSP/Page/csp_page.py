...
class CspPage(BasePage):
    ...
    other_add_csp =add_value[0]+'10' # 文件导入的csp名称
    ...
    def find_csp_data(self, name=add_value[0],csp_type=add_value[2]):
        """
        管理csp二级菜单查询出csp数据
        :param name: csp名称
    ...
    def update_csp(self, value):
        ...
        self.wait(2)
        if value == '1':  # 变更客户类型
            self.handle_select(self.update[1][2], self.update[2][2], self.update_value[0])
            ...
            self.handle_select(self.update[1][3], self.update[2][3], self.update_value[1])
    ...
    def new_csp(self, name=add_value[0]):
        ...
        try:
            ...
            self.input_value(self.add[1][27], self.add[2][27], self.add_value[22])  # 处理注册地下拉框
            self.wait(1)
    ...
    def intocheck(self, way='0'):
        ...
        if way == '0':
            ...
            if self.isElementExist(self.check[1][2], self.check[2][2]):
    ...
    def find_check_csp(self, cspname=add_value[0],way='0',check_csp_type=add_value[2],check_way=check_value[3],check_result=check_value[6]):
        ...
        :param check_csp_type:查询的csp类型 默认第三方csp
    ...
    def look_update(self, text):
        ...
        if text == '1':  # 查看token变更内容
            ...
            token_value = self.isElementExist(self.update[1][11], self.update[2][11])
            self.wait(1)
            return token_value

        elif text == '2':  # 查看接入密匙前后变更内容
            csp_log.csp_log.info(f'csp查看页面----------开始{self.update[0][10]}')
            code_value = self.isElementExist(self.update[1][10], self.update[2][10])
            self.wait(1)
            return code_value

        elif text == '3':  # 点击查看客户类型变更前后内容
            csp_log.csp_log.info(f'csp查看页面----------开始{self.update[0][12]}')
            self.isElementExist(self.update[1][12], self.update[2][12])
            self.wait(1)

        elif text == '4':  # 点击查看csp类型变更前后内容
            csp_log.csp_log.info(f'csp查看页面----------开始{self.update[0][13]}')
            self.isElementExist(self.update[1][13], self.update[2][13])
            self.wait(1)
    ...
    def look_gronding(self,result=search_value[1]):
        """
        查询已经上架或者下架的csp
        :param result:
        ...
        self.input_value(self.search[1][3], self.search[2][3], self.add_value[0])  # 输入csp名称
        self.wait(1)
    ...
    def other_add(self,add_name=other_add_csp):
        """
        ...
        try:
            ...
            self.upload_file(self.add[1][60], self.add[2][60],self.add_legal_value[19])  # 上传文件
            self.wait(1)
