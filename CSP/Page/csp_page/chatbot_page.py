...
chatbot_log=Log(__name__,file=logging.INFO,cmd=logging.WARN)
...
class  ChatbotPage(BasePage):
    ...
    search_value = [search_data.get_excel(8,5),      #chatbot接入类型
    ...
    update_value=[update_data.get_excel(2,5),     #0chatbot变更后的名称
                  update_data.get_excel(5,5),     #1chatbot变更后的速率
                  update_data.get_excel(10,5),    #2chatbot变更后的归属csp
                  update_data.get_excel(17,5),    #3chatbot变更后的行业类型
                  update_data.get_excel(18,5),    #4chatbot变更后的签名
                  update_data.get_excel(21,5),    #5chatbot变更后的归属客户名称
                  ]
    ...
    add_value = [add_data.get_excel(3,5),   #0输入chatbot归属csp名称
                 ...
                 add_data.get_excel(33,5),  #15备用的数据
                 ]
    ...
    def intoform(self):
        ...
        :return:
        """
        chatbot_log.csp_log.info(f'进入chatbot管理内置表单---------开始{self.searchdata[0][1]}')    #点击chatbot管理一级菜单
        self.click_element(self.searchdata[1][1],self.searchdata[2][1])
        chatbot_log.csp_log.info(f'进入chatbot管理内置表单---------开始{self.searchdata[0][2]}')    #点击chatbot审核二级菜单
    ...
    def add_chatbot(self,name=add_value[2]):
        ...
        try:
            ...
            self.input_value(self.add_msg[1][22],self.add_msg[2][22],int(self.add_value[7]))  #输入Chatbot服务电话
            self.wait(0.5)
    ...
    def find_chatbot(self,chatbotname=add_value[2],chatbot_type=search_value[0]):
        """
        csp管理二级菜单输入chatbot名称和选择chatbot接入类型查询数据
        :param chatbotname:
        :param chatbot_type:chatbot接入类型
        :return:
        """
        self.intoform()
        chatbot_log.csp_log.info(f'chatbot管理查询页面----------{self.searchdata[0][4]}')
        self.input_value(self.searchdata[1][4],self.searchdata[2][4],chatbotname)  #输入chatbot名称
        self.wait(0.5)
        chatbot_log.csp_log.info(f'chatbot管理查询页面----------{self.searchdata[0][8]}')
        self.wait(0.5)
        self.handle_select(self.searchdata[1][8],self.searchdata[2][8],chatbot_type)
        self.wait(0.5)
        self.click_element(self.searchdata[1][10],self.searchdata[2][10])  #点击查询按钮
        self.wait(0.5)

    def find_update_chatbot(self,chatbotcsp=update_value[2],chatbotcustomer=update_value[5],chatbotname=other):
        """
        查询变更信息的chatbot
        :param chatbotcsp:
        :param chatbotcustomer:
        :param chatbotname:
        :return:
        """
        self.intoform()
        ...
        self.input_value(self.searchdata[1][4], self.searchdata[2][4], chatbotname)  # 输入chatbot名称
        self.wait(0.5)
        chatbot_log.csp_log.info(f'chatbot管理查询页面----------{self.searchdata[0][7]}')
        self.input_value(self.searchdata[1][7], self.searchdata[2][7], chatbotcsp)  # 输入chatbot归属csp
        self.wait(0.5)
        chatbot_log.csp_log.info(f'chatbot管理查询页面----------{self.searchdata[0][6]}')
        self.input_value(self.searchdata[1][6], self.searchdata[2][6], chatbotcustomer)  # 输入chatbot归属csp
        self.wait(0.5)
    ...
    def update_name(self,updatename=update_value[0]):
        """
        变更chatbot名称与行业类型与签名
        :param updatename:
        :return:
        ...
        self.wait(2)
        chatbot_log.csp_log.info(f'chatbot信息查看页面-------{self.searchdata[0][1]}')
        ...
        chatbot_log.csp_log.info(f'chatbot信息查看页面-------{self.searchdata[0][2]}')
        self.input_value(self.updatedata[1][2], self.updatedata[2][2],updatename)  # 输入新的chatbot名称
        chatbot_log.csp_log.info(f'chatbot信息查看页面--------{self.updatedata[0][17]}')
        self.wait(0.5)
        self.handle_select(self.updatedata[1][17],self.updatedata[2][17],self.update_value[3])
        self.wait(0.5)
        chatbot_log.csp_log.info(f'chatbot信息查看页面--------{self.updatedata[0][18]}')
        self.clear_input(self.updatedata[1][18],self.updatedata[2][18])
        self.wait(0.5)
        self.input_value(self.updatedata[1][18],self.updatedata[2][18],self.update_value[4])
        self.wait(0.5)
        chatbot_log.csp_log.info(f'chatbot信息查看页面-------{self.searchdata[0][3]}')
        self.click_element(self.updatedata[1][3], self.updatedata[2][3])  # 点击确认变更按钮
        self.wait(0.5)
        self.F5()

    def update_speed(self,speed=update_value[1]):
        """
        变更chatbot速率
        :param speed
        :return:
        ...
        self.input_value(self.updatedata[1][5],self.updatedata[2][5],speed) #输入变更的chatbot速率
    ...
    def update_csp(self,updatecsp=update_value[2],updatecustomer=update_value[5]):
        """
        变更chatbot归属csp与归属客户
        :param updatecsp:
        :param updatecustmoer:
        ...
        chatbot_log.csp_log.info(f'chatbot信息查看页面-------{self.searchdata[0][1]}')
        self.click_element(self.updatedata[1][1], self.updatedata[2][1])  # 点击chatbot信息编辑按钮
        chatbot_log.csp_log.info(f'变更归属csp')
        self.click_element(self.updatedata[1][9],self.updatedata[2][9])
        self.wait(0.5)
        self.input_value(self.updatedata[1][10],self.updatedata[2][10],updatecsp)
        self.wait(0.5)
        self.click_element(self.updatedata[1][11], self.updatedata[2][11])
        self.wait(0.5)
        self.click_element(self.updatedata[1][12], self.updatedata[2][12])
        self.wait(0.5)
        self.click_element(self.updatedata[1][13], self.updatedata[2][13])
        self.wait(2)
        chatbot_log.csp_log.info(f'变更归属客户')
        self.click_element(self.updatedata[1][14], self.updatedata[2][14])
        self.wait(0.5)
        self.input_value(self.updatedata[1][21],self.updatedata[2][21],updatecustomer)
        self.wait(0.5)
        self.click_element(self.updatedata[1][22],self.updatedata[2][22])
        self.wait(0.5)
        self.click_element(self.updatedata[1][15], self.updatedata[2][15])
        self.wait(0.5)
        self.click_element(self.updatedata[1][16], self.updatedata[2][16])
        self.wait(2)
        chatbot_log.csp_log.info(f'chatbot信息查看页面-------{self.searchdata[0][3]}')
        self.click_element(self.updatedata[1][3], self.updatedata[2][3])  # 点击确认变更按钮
        self.wait(0.5)
    ...
    def intocheck(self,way:bool):
        ...
        self.click_element(self.searchdata[1][1],self.searchdata[2][1])
        chatbot_log.csp_log.info(f'进入chatbot审核内置表单------开始{self.checkdata[0][2]}')
    ...
    def find_check(self,check_chatbotname=add_value[2],check_type='信息创建',csp_type=check_value[0]):
        """
        查询出待审核的chatbot
        ...
        :return:
        """
        chatbot_log.csp_log.info(f'查询待审核的chatbot数据------{self.checkdata[0][4]}')
        self.input_value(self.checkdata[1][4], self.checkdata[2][4],check_chatbotname)  # 输入待审核的chatbot名称
        self.wait(0.5)
        chatbot_log.csp_log.info(f'查询待审核的chatbot数据------{self.checkdata[0][6]}')
        self.handle_select(self.checkdata[1][6],self.checkdata[2][6],check_type)  #选择审核类型
        self.wait(0.5)
        chatbot_log.csp_log.info(f'查询待审核的chatbot数据------{self.checkdata[0][5]}')
        self.wait(0.5)
        self.handle_select(self.checkdata[1][5],self.checkdata[2][5],csp_type)     #选择客户接入类型
        self.wait(0.5)
    ...
    def check_chatbot(self,result='0',check_way=checkdata[2][9]):
        """
        开始审核chatbot
        :param result: 默认审核通过
        :return:
        ...
        if result=='0':   #审核通过
            ...
            self.click_element(self.checkdata[1][9],check_way)  #点击审核不通过
            self.wait(1)
            chatbot_log.csp_log.info(f'chatbot审核页面---------{self.checkdata[0][10]}')
            self.input_value(self.checkdata[1][10],self.checkdata[2][10],self.check_value[1])  #输入名称变更审核不通过原因
            self.wait(1)
            chatbot_log.csp_log.info(f'chatbot审核页面---------{self.updatedata[0][19]}')
            self.input_value(self.updatedata[1][19],self.updatedata[2][19],self.random_number())                    #输入签名审核不通过原因
            self.wait(1)
            chatbot_log.csp_log.info(f'chatbot审核页面---------{self.updatedata[0][20]}')
            self.input_value(self.updatedata[1][20], self.updatedata[2][20],self.random_number())                   #输入行业类型变更审核不通过原因
            self.wait(1)
    ...
    def assert_find(self,findname,assert_text,assert_type=search_value[0]):
        """
        断言查询结果
        :param findname:
        :param assert_text:
        ...
        """
        value = assert_text
        self.find_chatbot(chatbotname=findname,chatbot_type=assert_type)
